...
`compute_score` combines recency, source weight, title keyword matches, and
HN engagement metrics to produce a sortable float score for export ranking.
"""
...
def compute_score(item: dict[str, Any], source_weight: float = 1.0) -> float:
    ...
    base = 1.0
    score = (base + recency + keyword_boost + hn_boost) * max(0.1, float(source_weight or 1.0))
